...
aha_setbios(x54x_t *dev)
{
    ...
    if ((f = rom_fopen(dev->bios_path, _S("rb"))) == NULL) {
...
aha_init(const device_t *info)
{
	...
	case AHA_154xA:
		...
		dev->bios_path = _S("roms/scsi/adaptec/aha1540a307.bin"); /*Only for port 0x330*/
	...
	case AHA_154xB:
		...
		switch(dev->Base) {
			case 0x0330:
				dev->bios_path =
				    _S("roms/scsi/adaptec/aha1540b320_330.bin");
				break;

			case 0x0334:
				dev->bios_path =
				    _S("roms/scsi/adaptec/aha1540b320_334.bin");
				break;
		}
	...
	case AHA_154xC:
		...
		dev->bios_path = _S("roms/scsi/adaptec/aha1542c102.bin");
		dev->nvr_path = _S("aha1542c.nvr");
		dev->fw_rev = "D001";
		dev->rom_shram = 0x3F80;	/* shadow RAM address base */
		dev->rom_shramsz = 128;		/* size of shadow RAM */
	...
	case AHA_154xCF:
		...
		dev->bios_path = _S("roms/scsi/adaptec/aha1542cf211.bin");
		dev->nvr_path = _S("aha1542cf.nvr");
		dev->fw_rev = "E001";
		dev->rom_shram = 0x3F80;	/* shadow RAM address base */
		dev->rom_shramsz = 128;		/* size of shadow RAM */
		dev->rom_ioaddr = 0x3F7E;	/* [2:0] idx into addr table */
	...
	case AHA_154xCP:
		...
		dev->bios_path = _S("roms/scsi/adaptec/aha1542cp102.bin");
		dev->nvr_path = _S("aha1540cp.nvr");
		dev->fw_rev = "F001";
		dev->rom_shram = 0x3F80;	/* shadow RAM address base */
		dev->rom_shramsz = 128;		/* size of shadow RAM */
		dev->rom_ioaddr = 0x3F7E;	/* [2:0] idx into addr table */
	...
	case AHA_1640:
		...
		dev->bios_path = _S("roms/scsi/adaptec/aha1640.bin");
		dev->fw_rev = "BB01";
